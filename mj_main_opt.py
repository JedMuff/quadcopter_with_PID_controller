...
def obj_fun(params, target=2.0):
    ...
    quadcopter = Quadcopter(pos,vel,ang,ang_vel,r_ref, 0.01)
    ...
    obj_val = (settling_time()-target)**2
...
for method in methods: #'Powell', 'Nelder-Mead', 'COBYLA', 'SPSA', 'EGO', 'CMA-ES', 'DE', 'PSO'
    if method in ['Powell','Nelder-Mead','COBYLA']:
        ...
        res=opal.de(obj_fun2, len(params0), X=None, bounds=bounds, gaptol=1.0e-06, pop=30, Cr=0.65, F=0.75, dither=True, IniNfeval=0, maxiter=5000, endMinf=1e-6, endMaxf=1e-6, maxNfeval=100, args=())
        params=res[0]
        res_his=np.array(res[5])[:,1]
